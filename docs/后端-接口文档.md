...
- PUT /orgs/{orgId}/leases/{leaseId}（lease.write）
  ...
  - 逻辑：校验日期、检查租约重叠、更新租约、根据该房间是否存在 ACTIVE/DRAFT 租约自动同步房间 isRented 状态
  - 响应：{ lease }
...
- GET /orgs/{orgId}/invoices（billing.read）
  ...
  - 响应：{ invoices(include lease(room(apartment), tenant), items, hasPendingReading) }
...
- POST /orgs/{orgId}/invoices/{invoiceId}/confirm（billing.manage）
  - 动作：确认账单（要求所有待确认读数项已确认，然后将账单状态从 DRAFT 更新为 ISSUED）
  - 错误：PENDING_READINGS（存在待确认的读数）
  - 响应：{ ok: true }
- GET /orgs/{orgId}/invoices/{invoiceId}/export（billing.read）
  - 响应：Excel 文件下载（Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet）
- POST /orgs/{orgId}/invoices/{invoiceId}/mark-paid（billing.manage）
  - 动作：标记账单为已支付
  - 响应：{ ok: true }
...
- POST /orgs/{orgId}/signing（lease.write）
  - Body：{ tenantId?, newTenant?: { name, phone, idNumber? }, roomId, startDate, endDate, baseRentCents, billingCycleMonths?, depositCents?, rentIncreaseType?, rentIncreaseValue?, rentIncreaseIntervalMonths?, notes?, charges?[] }
  - 校验：必须提供 tenantId 或 newTenant 之一；时间不重叠；费用项 FIXED/METERED 参数完整
  - 响应：{ message: '签约成功', lease(include room(apartment), tenant, charges), tenant }

## 仪表盘模块 [dashboard.router.ts](file:///e:/learn_file/cursor_project/apartment-manager/apps/api/src/modules/dashboard/dashboard.router.ts)

- GET /orgs/{orgId}/dashboard/vacant-rooms（dashboard.read）
  - 响应：{ rooms(include apartment), asOf }
  - 说明：返回当前未租出的激活房间列表
- GET /orgs/{orgId}/dashboard/lease-expiring（dashboard.read）
  - Query：days?（默认30，最大365）
  - 响应：{ leases(include room(apartment), tenant), now, until }
  - 说明：返回即将到期的租约列表
- GET /orgs/{orgId}/dashboard/kpis（dashboard.read）
  - 响应：{ asOf, kpis: { apartmentCount, totalRoomCount, occupiedRoomCount, occupancyRate, invoiceIssuedCount, invoiceIssuedTotalCents, invoiceOverdueCount, invoiceOverdueTotalCents } }
