import { Button, Divider, Form, InputNumber, Modal, Space, Typography } from 'antd';
...
export function PricingPlanModal({ open, room, onClose, onSuccess }: PricingPlanModalProps) {
  ...
  const feePricingsQuery = useQuery({
    queryKey: queryKeys.apartments.feePricings(orgId!, room?.apartmentId ?? ''),
    enabled: !!orgId && !!room?.apartmentId && open,
    queryFn: async () => {
      if (!orgId || !room?.apartmentId) throw new Error('Missing orgId or apartmentId');
      return apartmentsApi.getFeePricings(orgId, room.apartmentId);
    },
  });
    ...
    >
      <Space direction="vertical" style={{ width: '100%' }} size="large">
        ...
        <div>
          ...
          {feePricingsQuery.isLoading ? (
            <Typography.Text type="secondary">加载中...</Typography.Text>
          ) : feePricings.length === 0 ? (
            <Typography.Text type="secondary">该公寓暂未配置费用项目</Typography.Text>
          ) : (
            <FeePricingTable feePricings={feePricings} loading={feePricingsQuery.isLoading} />
          )}
        </div>
      </Space>
    </Modal>
